...
namespace RestSharp.Deserializers {
   ...
   public class JsonRpcDeserializer : IDeserializer {
      ...
      public T Deserialize<T> ( IRestResponse response ) {
         object json = this.FindResult( response.Content );

         return ( T ) this.ConvertValue( typeof( T ), json );
      }

      private object FindResult ( string content ) {
         ...
         if ( dictionary != null ) {
            object result;
            if ( dictionary.TryGetValue( "result", out result ) ) {
               return result;
            } else if ( dictionary.TryGetValue( "error", out result ) ) {
               throw ParseFault( result );
            }
         }
      ...
      private static JsonRpcFaultException ParseFault ( object fault ) {
         IDictionary<string, object> dictionary = fault as IDictionary<string, object>;

         var faultCode = dictionary["code"].ToString();
         var faultMessage = ( string ) dictionary["message"];

         int fc;
         var faultCodeValue = faultCode != null && int.TryParse(faultCode, out fc) ? int.Parse(faultCode) : -1 ;
         string faultStringValue = faultMessage ?? "unknown";

         return new JsonRpcFaultException( faultCodeValue, faultStringValue );
      }
   }
}
